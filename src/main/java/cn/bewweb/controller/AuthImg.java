...
public class AuthImg {
	...
	// authCode
	@RequestMapping(value = "/checkAuthImg", method = RequestMethod.POST)
	public @ResponseBody Json checkAuthImg(String authCode, HttpServletRequest request) {
		// System.err.println(authCode);
		HttpSession session = request.getSession();
		if ((authCode == null || "".equals(authCode)) ? true : false) {
			return (new Json()).setCode(Code.lostParam);
		}
		authCode = authCode.trim();
		String imgRandCode = (String) session.getAttribute("rand");
		// System.err.println(imgRandCode);
		if (imgRandCode.equalsIgnoreCase(authCode)) {
			session.setAttribute("imgValid", true);
			return (new Json()).setCode(Code.c1);
		} else {
			session.setAttribute("imgValid", false);
